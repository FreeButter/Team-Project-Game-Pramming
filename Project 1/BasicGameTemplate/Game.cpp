	...
	if (m_gameState == m_gameStates::LOST)
	{
		...
		if (m_gameState == m_gameStates::PLAYING)
		{
			...
			for (m_entityIt = m_entityVector.begin(); m_entityIt != m_entityVector.end(); ++m_entityIt)
			{
				...
				{
					bool ballLimit = 20;
					if (m_ballVector.size() <= ballLimit + 1)
					{
						SpawnBall(e);
					}
			...
			for (m_ballIt = m_ballVector.begin(); m_ballIt != m_ballVector.end(); ++m_ballIt)
			{
				...
				for (m_entityIt = m_entityVector.begin(); m_entityIt != m_entityVector.end(); ++m_entityIt)
				{
					...
					if (collided)
					{
						...
						if (lives <=0 && player->IsCollided()==false){					
...
void Game::Render()
{
	...
	// TODO: Render Text with content and placement based from the json file
	// Render the text
	float xPercent = 0.7;
	float yPercent = 0.1;
	char intStr[30];
	sprintf(intStr, "%f", m_countdown);
	m_textController->Render(DirectX::SimpleMath::Vector2(m_windowWidth, m_windowHeight),
		xPercent, yPercent, intStr, DirectX::Colors::White);
	...
	{
		// TODO: RENDERING DEBUG CODE
		m_countdown = (float)m_countdown - m_timer.GetElapsedSeconds();
